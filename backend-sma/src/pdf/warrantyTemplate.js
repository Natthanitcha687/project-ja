...
export function drawWarrantyPage(doc, base, item) {
  ...
  // Header: logo centered at top, then title underneath (matches sample)
  const logoSize = mm(36);
  ...
  export function drawWarrantyPage(doc, base, item) {
    ...
    export function drawWarrantyPage(doc, base, item) {
      ...
      headerTitle(doc, left, top + mm(20), width, fonts);

      const tableTop = top + mm(44);
      const tableW = width;
      ...
      // โลโก้ + ข้อมูลบริษัท (ด้านล่าง)
      try {
        const candidates = [
          path.resolve(process.cwd(), "src/assets/logo.png"),
          new URL("../assets/logo.png", import.meta.url).href,
        ];
        const logoBottom = resolveFirstExisting(candidates);
        if (logoBottom) doc.image(logoBottom, left, A4.h - mm(44), { width: mm(18) });
      } catch {}
